...
def format_data(data_input: str, column: str, excludestop: bool) -> pd.DataFrame:
    pandas_df = pd.read_csv(data_input)
    pandas_df['POSITION'] = pandas_df['POSITION'] - pandas_df['POSITION'].min() + 1
    
    if excludestop:
            pandas_df = pandas_df['AA' != '.']
            
    positions_to_keep = pandas_df.groupby('POSITION')[column].apply(lambda x:(x > 0).any())
    positions_to_keep = positions_to_keep[positions_to_keep | ~positions_to_keep]        
    pandas_df = pandas_df[pandas_df['POSITION'].isin(positions_to_keep.index)]
    
    info_df = pandas_df[['POSITION', 'AA', column]]
    info_pivot_matrix = info_df.pivot(index="POSITION", columns="AA", values=column)
    info_pivot_matrix.fillna(0, inplace=True)
    return info_pivot_matrix

def generate_logo_plot(matrix_input: pd.DataFrame, sample_name: str, y_title: str, annotations: str, increment: int, min_label_len: int, output_path: str, codon_start: int = None, codon_end: int = None, ref_aa_path: str = None) -> None:
    ...
    def match_annotation(ax, position_start: int, position_end: int, min_label_len: int) -> None:
        ...
        if annot_start == annot_end:
            ax.plot([position_start-1, position_end+1], [max_value*1.15, max_value*1.15], alpha=0.5, color=annot_data[annot_start]['color'], linewidth=12, solid_capstyle="butt")
            if (position_end - position_start) >= min_label_len:
                ax.text(((position_start-1 + position_end+1) / 2), max_value*1.1, annot_data[annot_start]['region_name'], fontsize=10, color='black')
        else:
            for subannotations in enumerate(range(annot_start, annot_end + 1)):
                if subannotations[0] == 0:
                    ...
                    ax.plot([begin-1, end+1], [max_value*1.15, max_value*1.15], alpha=0.5, color=annot_data[annot_start]['color'], linewidth=12, solid_capstyle="butt")
                    if (end - begin) >= min_label_len:
                        ax.text(((begin-1 + end+1) / 2), max_value*1.1, annot_data[annot_start]['region_name'], fontsize=10, color='black')
                    ...
                    ax.plot([begin, end+1], [max_value*1.15, max_value*1.15], alpha=0.5, color=annot_data[annot_end]['color'], linewidth=12, solid_capstyle="butt")
                    if (end - begin) >= min_label_len:
                        ax.text(((begin + end+1) / 2), max_value*1.1, annot_data[annot_end]['region_name'], fontsize=10, color='black')
                else:
                    begin = annot_data[subannotations[1]]['start']
                    end = annot_data[subannotations[1]]['end']
                    ax.plot([begin, end+1], [max_value*1.15, max_value*1.15], alpha=0.5, color=annot_data[subannotations[1]]['color'], linewidth=12, solid_capstyle="butt")
                    if (end - begin) >= min_label_len:
                        ax.text(((begin + end+1) / 2), max_value*1.1, annot_data[subannotations[1]]['region_name'], fontsize=10, color='black')
    
    
    max_value_positive_only = matrix_input[matrix_input > 0].sum(axis=1).max()
    max_value_all_included = matrix_input.sum(axis=1).max()
    max_value = round(max(max_value_positive_only, max_value_all_included))
    y = max_value*1.5
    ...
    for i in range(0, num_rows):
        ...
        logomaker.Logo(tmp, ax=ax, color_scheme="skylign_protein", show_spines=True, stack_order="big_on_top")
        ax.set_ylim([0, y])
        ax.set_yticks([0, max_value])
        ax.set_yticklabels(['0', f'{max_value:.0f}'])
        ...
        if ref_aa_data is not None:
            ...
            for _, row in ref_aa_subset.iterrows():
                ax.text(row['POSITION'], max_value*1.3, row['REF_AA'], fontsize=10, color='black', ha='center')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sampleName', type=str, default="sample", help="String indicating the name to give to sample")
    parser.add_argument('--yTitle', type=str, required=True, help="String indicating title to give to y-axis")
    parser.add_argument('--column', type=str, choices=['MERGE_FRAC', 'mutdiffsel'], required=True, help="Column to use for plotting")
    ...
    parser.add_argument('--excludeStop', action='store_true', help="Exclude rows where AA is '.'")


    args = parser.parse_args()

    formatted_input_matrix = format_data(data_input=args.input, column=args.column, excludestop=args.excludeStop)
    generate_logo_plot(matrix_input=formatted_input_matrix, sample_name=args.sampleName, y_title=args.yTitle,
